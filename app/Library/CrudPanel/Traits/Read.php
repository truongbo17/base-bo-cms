...
trait Read
{
     ...
     */
    public function getEntry($id, $with_soft_delete = false)
    {
        if (! $this->entry) {
            $this->entry = $this->getModelWithCrudPanelQuery();

            if ($with_soft_delete){
                $this->entry = $this->entry->withTrashed();
            }
            $this->entry = $this->entry->findOrFail($id);
